...
def process_train_nq_dataset(dataset):
    ...
    for idx, item in enumerate(dataset):
        ...
        golden_answers = item.get("Final answer", [])
...
BENCHMARK_MAPPING = {
    'gaia_text_only': 'default'  # 修改为 'default'，因为数据集只提供默认配置
}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--benchmarks', type=str, default='gaia_text_only',
    ...
    if args.generate_test:
        ...
        for benchmark_name in benchmark_list:
            ...
            try:
                # 加载 GAIA 数据集，使用 'default' 配置
                dataset = datasets.load_dataset('sayan1101/gaia_filtered_text_only', dataset_name)
                ...
                def make_map_fn(data_source):
                    def process_fn(example, idx):
                        ...
                        print("Final answer",example['Final answer'])
                        solution = {
                            "target": example['Final answer'],
                        }
